...
import { process_exit_ } from '../activity/exporter.js'
import { shell_exit_codes__ } from '../errors/exporter.js'
    ...
    value: async function* selector ( process_parsed_argv ){
                    ...
                    switch ( process_parsed_argv.keys[ flag ] ) {
                        ...
                        case 'address':
                            ...
                            process_parsed_argv.object[ process_parsed_argv.keys[ flag ] ] = await address_flag_get(
                                ...
                                .catch( async error => {
                                    await process_exit_( '--address accept only string.', error, shell_exit_codes__.flags )
                                } )
                        ...
                        case 'cluster':
                            ...
                            process_parsed_argv.object[ process_parsed_argv.keys[ flag ] ] = await cluster_flag_get(
                                ...
                                .catch( async error => {
                                    await process_exit_( '--cluster accept only numbers or string "full".', error, shell_exit_codes__.flags )
                                } )
                        ...
                        case 'domain':
                            ...
                            process_parsed_argv.object[ process_parsed_argv.keys[ flag ] ] = await domain_flag_get(
                                ...
                                .catch( async error => {
                                    await process_exit_( '--domain accept only string.', error, shell_exit_codes__.flags )
                                } )
                        ...
                        case 'logger': {
                            ...
                            const logger = await logger_flag_get( process_parsed_argv.object[ process_parsed_argv.keys[ flag ] ] )
                                .catch( error => process_exit_( error, Error( `${ process.title } flags-error` ), shell_exit_codes__.flags ) )
                        ...
                        case 'port':
                            ...
                            if( await undefined_( process_parsed_argv.object[ process_parsed_argv.keys[ flag ] ] ) === false ) {
                                ...
                                if ( await number_( process_parsed_argv.object[ process_parsed_argv.keys[ flag ] ] ) === false )
                                    await process_exit_( '--port accept only numbers.', Error( `${ process.title } flags-error` ), shell_exit_codes__.flags )
                        ...
                        default: {
                            ...
                            await process_exit_( error, new TypeError( `${ process.title } flags-error` ), shell_exit_codes__.flags )
                        }
                    }
                    ...
                    switch ( process_parsed_argv.keys[ flag ] ) {
                        ...
                        case 'init': {
                            ...
                            await init_command_get( process_parsed_argv.object[ process_parsed_argv.keys[ flag ] ] )
                                .catch( error => process_exit_( error.message, Error( `${ process.title } commands-error` ), shell_exit_codes__.commands ) )
                        ...
                        default: {
                            ...
                            error += `\x1b[41m        run -> ${ process.title } help[h]        \x1b[0m\n`
                            await process_exit_( error, new TypeError( `${ process.title } commands-error` ), shell_exit_codes__.commands )
                        }
        ...
        yield new Promise( ( resolve, reject ) => {
            ...
            if( no_process ) {
                ...
                process_exit_( error, new ReferenceError( 'internal-error' ), shell_exit_codes__.internal ).catch( error => {throw error} )
            }
