...
const processor = Object.defineProperty( input, processorSymbol, {
    ...
    value: async function processor( argv ) {
         ...
         *    ./koorie --secure=options(active:true:key:certs/some-key.pem)
         *    the string 'certs/some-key.pem' will be replaced by 'certs/some_key.pem'
         *    and in case of --secure the server will crash on boot because will not find the key file.
