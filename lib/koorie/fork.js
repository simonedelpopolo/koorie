...
import { cluster_types, process_exit, resource_get_public, shell_exit_codes } from '../../index.js'
...
const fork = Object.defineProperty( koorie, forkSymbol, {
    ...
    value: async function fork( cpus, static_files, ejected ){
        ...
        if( await oftype_( ejected ) !== 'undefined' ) {
            ...
            let ejected = 'koorie.js'
            if( process.env.EJECTED !== 'false' &&  typeof process.env.EJECTED_FILE !== 'undefined' )
                ejected = process.env.EJECTED_FILE
            initializeFile = `${ process.cwd() }/${ ejected }`
        }
        ...
        if ( cluster.isPrimary ) {
            ...
            cluster.on( 'message', async ( worker, options ) => {
                ...
                process.argv.push( '--false-flag=true' )
                process.argv.push( `--static-files=${resource_get_public()}` )
