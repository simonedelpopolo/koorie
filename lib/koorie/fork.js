...
export const fork = Object.defineProperty( koorie, forkSymbol, {
    ...
    value: async function cluster_( cpus, static_files ){
        ...
        /* - const socket_argv_index = process.argv.indexOf( `--socket=options(active:${process.env.SOCKET_ACTIVE}:path:${process.env.SOCKET_PATH})` )
        if( socket_argv_index > -1 )
            process.argv.splice( socket_argv_index, 1 )*/
        ...
        // - socket[ socketSymbol ]( { path: process.env.SOCKET_PATH } )
        ...
        if ( cluster.isPrimary ) {
            ...
            cluster.on( 'message', async ( worker, message ) => {
                ...
                // - process.argv.push( `--hot=${message.hot}` )
