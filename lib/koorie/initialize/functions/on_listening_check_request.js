...
import { Answer, process_exit, routes_collection, routes_inject, routes_injected, routes_set, shell_exit_codes } from '../../../../index.js'
...
export async function on_listening_check_request(){
    ...
    async function on_listening( Incoming, Outgoing ){
        ...
        const get_params = await Answer.toGet( await Answer.getQuestion( 'params' ), 'on_listening_get_params' )
        const body = await Answer.toPost( await Answer.getQuestion( 'body'  ), 'on_listening_get_params' )
    ...
    const request = on_listening_check_request( options, response => {
        ...
        response.on( 'end', async () => {
            ...
            const actual = {
                ...
                'on-listening-request': await true_false( response.headers[ 'on-listening-request' ] ),
                'on-listening-params' : response.headers[ 'on-listening-params' ],
                'on-listening-body' : response.headers[ 'on-listening-body' ],
                'response': await parse( Buffer.concat( buffer ) )
            }
            ...
            try{
                ...
                await process_exit( 'something went wrong with the on_listening_check', error, shell_exit_codes.internal )
            }
        } )
