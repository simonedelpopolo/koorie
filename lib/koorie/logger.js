...
export const logger = Object.defineProperty( koorie, loggerSymbol, {
    ...
    value: async function logger( options ){
        ...
        if( await string_( log.write ) === true ) {
            const accessDir = await access( dirname( log.write ), constants.F_OK || constants.W_OK ).catch( error => error.message )
            
            if( await string_( accessDir ) === true )
                await process_exit( accessDir, Error( 'Koorie-Terminal-Error' ), 2 )
    
            for( const message in log.info )
                
                await writeFile( log.write, Buffer.from( JSON.stringify( log.info[ message ] ) + '\n' ), { flag: 'a+' } )
