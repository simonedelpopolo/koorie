...
import { request_id, request_timing, time_ } from './dispatcher.js'
...
const outgoing = Object.defineProperty( koorie, outgoingSymbol, {
    ...
    value: async function outgoing( response, outgoing ){
        ...
        let response_time
        if( ( await string_( response.gone, undefined, true ) )[ 1 ] === 'wrote' ) {
        ...
        if( await oftype_( response.log ) !== 'undefined' && process.env.SILENCED !== 'true' && process.env.FIRST !== 'true' ) {
        ...
        if( await true_false( process.env.RESPONSE_TIME ) === true  && process.env.SILENCED !== 'true' && process.env.FIRST !== 'true' ) {
