                ...
                rl.question( 'public path is set to the root directory. is this fine? [yes|no]\n'.red(), async ( answer ) => {
                    ...
                    switch ( path_length ){
                        ...
                        case true:
                            ...
                            break
                        default:
                            await process_exit( 'only [yes|y] OR [no|n] are accepted answer'.bg_red().color( 255 ).strong().underline(), new ReferenceError( `[koorie.resource.set_public() - boolean-jokes.map()]\n${path_length.bg_magenta().color( 255 ).strong().underline()}` ), shell_exit_codes.flags )
                            break
                    }
                } )
            }else
