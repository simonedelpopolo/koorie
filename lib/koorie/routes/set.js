...
import { process_exit, request_routes, routes_collection, routes_injected, routes_set_check, shell_exit_codes } from '../../../index.js'
    ...
    value:  async function set (){
        ...
        for ( const route in routes_collection ){
            ...
            if( routes_collectionKeys.includes( 'asyncFunction' ) && routes_collectionKeys.includes( 'syncFunction' ) )
                await process_exit( 'only one property asyncFunction OR syncFunction, not both', new TypeError( 'koorie.routes.set' ), shell_exit_codes.type_checking )

            if( ! ( routes_collectionKeys.includes( 'asyncFunction' ) ) && !( routes_collectionKeys.includes( 'syncFunction' ) ) )
                await process_exit( 'injection of routes require a property set to asyncFunction OR syncFunction', new TypeError( 'koorie.routes.set' ), shell_exit_codes.type_checking )
            ...
            let result_check
            result_check = await function_type_check.next().then( result => result.value )

            if( await oftype_( result_check ) !== 'Function' )
                result_check = await function_type_check.next().then( result => result.value )

            const RouteExport = await function_type_check.return( result_check ).then( final_result => final_result.value )

            if( RouteExport === false )
                await process_exit( `given ${function_type} isn't an AsyncFunction neither a Function. given type -> ${await oftype_( routes_collection[ route ][ function_type ] )}`, new TypeError( 'koorie.routes.set' ), shell_exit_codes.type_checking )
