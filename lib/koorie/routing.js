...
const routing = Object.defineProperty( koorie, routingSymbol, {
    ...
    value: async function routing( parameters ){
        ...
        if( routesKeys.includes( route[ 0 ] ) ){
            ...
            await request_query( routeUrl )

            if( route.length > 0 ){
                for ( const path in route ){
                    if( request_routes.includes( route[ path ] ) === false )
                        request_routes.push( route[ path ] )
                }
            }
            ...
            if ( resource_get_images().includes( resourceExt ) ) {
            ...
            if( resource_get_text().includes( resourceExt ) ) {
            ...
            if( resource_get_application().includes( resourceExt ) ) {
        ...
        }else if( javascript_library === 'no_route' ){
            responseReady = false
            outgoing.statusCode = 500
            outgoing.statusMessage = 'Not Found'
            outgoing.setHeader( 'koorie', 500 )
