...
export let routing = Object.defineProperty( koorie, routingSymbol, {
    ...
    value: async function routing( parameters ){
        ...
        if( routesKeys.includes( route[ 0 ] ) ){
            ...
            console.trace( process.env.HOT )
            let routeAsyncFunction
    
            // - hot reloading of routes
            if( await true_false( process.env.HOT ) === true ) {
                const hot_route = route[ 0 ].length === 0 ? 'index' : route[ 0 ]
                // eslint-disable-next-line capitalized-comments
                // noinspection JSFileReferences
                routeAsyncFunction = ( await import( `${process.cwd()}/routes/${ hot_route }/route.js?${request_id}` ) )[ hot_route ]
            }
            else
                // If a route has been found, calls the AsyncFunction of the route
                routeAsyncFunction = await routes.get( route[ 0 ] )
