        ...
        install.on( 'exit', async code => {
            ...
            if( options.git ) {
                ...
                spawn_git_available.on( 'exit', async code => {
                    if ( code !== 0 ) {
                        console.warn( Blaze.bg_blue( Blaze.color( 255, 'git not available on host OS' ) ),
                            new ReferenceError( 'Object [react-dang.install] not-satisfied-error' ), error_code.COMMAND )
                    }else{
                        ...
                        git.on( 'exit', async code => {
                            if( code !== 0 ) {
                                console.warn(
                                    Blaze.color( 255, `git exited with code ➡ ${Blaze.bg_blue( code.toString( 10 ) )}` ),
                                    new ReferenceError( 'Object [react-dang.install] not-satisfied-error' ), error_code.COMMAND )
                            }else
                                console.log( Blaze.green( 'git init successful\r' ) )
