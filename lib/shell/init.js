    ...
    value : async function init( options ){
        ...
        async function generating(){
            ...
            await writeFile( `${working_dir}/package.json`, JSON.stringify( package_json ), { flag: 'w' } ).catch( async error => {
                await process_exit( error.message, new Error( 'Object [shell.init] writeFile `working_dir/package.json`' ), shell_exit_codes.internal_error )
            } )
            ...
            install.on( 'error', async error => {
                await process_exit( error.message, new Error( 'Object [shell.init] spawn `error event`' ), shell_exit_codes.internal_error )
            } )
    
            install.on( 'exit', async code => {
                if( code !== 0 )
                    await process_exit( `${'something went wrong with npm.'.red()} \n exit code ->${code}`, new Error( 'Object [shell.init] spawn `exit event error`' ), shell_exit_codes.internal_error )
        
                const package_json = await readFile( `${working_dir}/packagef.json` ).catch( async error => {
                    await process_exit( error.message, new Error( 'Object [shell.init] readFile `package.json`' ), shell_exit_codes.internal_error )
                } )
        
                await is_json( package_json, true ).catch( async error => {
                    await process_exit( error, new Error( 'Object [shell.init] is_json `error package.json not json`' ), shell_exit_codes.internal_error )
                } )
